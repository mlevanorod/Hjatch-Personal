...
<%
				...
				COMANDO = "SELECT "+
							"		CASE CODACT "+
							"				WHEN 'D' THEN DBO.CODIGO_ASISTENCIA_COMPLE(DIA, MES, "+anio+", CODIGO) "+
							"				ELSE CODACT "+
							"		 END CODACT, "+
							"		CASE CODACT "+
							"			WHEN 'D' THEN CASE DBO.CODIGO_ASISTENCIA_COMPLE(DIA, MES, "+anio+", CODIGO) "+
							"							WHEN 'D' THEN '#FF0000'  "+
							"							ELSE '#000000' "+
							"						   END  "+
							"			 ELSE '#0000FF' "+
...
<%
								if( (min == 0 || cod.equals("D")) || ( e==1 || (min == 0 || cod.equals("D"))  ) ) 
								{
%>								
								<select class="input-sm form-control" style="color:<%=rset.getString("CODACT_COLOR")%>;  background:white; fxont-weight:bold" id="dia_<%=e%>" onchange="myFunction(<%=e%>)" name="dia_<%=e%>" >
                    <%
                                ...
                                {
                                	COMANDO2 += " WHERE TIPO_HORA IN ('T','V','F') AND TIPO_HORA <> 'N' ";
                            	}else{

                            		COMANDO2 += " WHERE TIPO_HORA IN ('N','V','F') AND TIPO_HORA <> 'T' ";
...
%>
									<font color="<%=rset.getString("CODACT_COLOR")%>" size=1><b> <%=cod%> </b></font> </br>
<%
								}
%>                                
...
%>
                    ...
                    <%
                                ...
                                if("20".equals(situacion) || "06".equals(situacion))
                                {
                                	COMANDO2 += " WHERE TIPO_HORA IN ('T','V','F') ";
                            	}else{

                            		COMANDO2 += " WHERE TIPO_HORA IN ('N','V','F') ";
...
<%
		COMANDO = "  select		CODHOR, "+
                    ...
                    if("20".equals(situacion) || "06".equals(situacion))
                    {
                    	COMANDO += " WHERE TIPO_HORA IN ('T','V','F') ";
                	}else{

                		COMANDO += " WHERE TIPO_HORA IN ('N','V','F') ";
