...
%>
						...
						<tr>
							<td colspan="35">
								(**) PERSONAL INCIO LABORES COMO CAS, DESDE EL 27-05-2020 ->
							</td>
						</tr>
<%
						}
...
<%
						if("2.0".equals(estado) || "3.0".equals(estado) || "4.0".equals(estado))
						{
							COMANDO = "select top 1 ltrim(rtrim(DNI)) AS DNI from [BDPERSONAL].[dbo].[FIRMA_JEFE_DEP] WHERE ID_DEPART = (SELECT IdDepartUnid FROM ServicioDepartUnidad WHERE IdServDepartUnid = '"+s_unidad+"' and IdDepartUnid <> 7) AND INICIO < '15-"+s_mes+"-"+s_anio+"' AND FIN > '15-"+s_mes+"-"+s_anio+"'";
							//out.print(COMANDO);
							rset = stmt.executeQuery(COMANDO);
							if(rset.next())
							{
