...
<!-- Javascript -->
	...
	<script>
        ...
        {
            //var cod_act = $(this).attr('name');
            
            $( "#mostrar4" ).load( 'asignar_actividad.jsp?f_codigo='+cod_act+'&f_anio='+$('#f_anio').val()+'&f_mes='+$('#f_mes').val()+'&f_dia='+dia );
        }


		init.push(function () {

			//$('#reporte').fadeIn("fast").load('reporte.jsp?f_mes='+$('#f_mes').val()+'&f_unidad=0'+'&f_anio='+$('#f_anio').val()+'&f_id_personal='+$('#f_id_personal').val()+'&f_actividad=0', function(){ $("#cargando").css("display", "none");});
            ...
            $('#f_actividad').change(pressActividad);
            $("#f_servicio").hide();
            $("#f_actividad").hide();
            ...
            $(document).delegate("#f_unidad","change",function(){
                ...
                $.post('servicio.jsp?f_unidad='+$('#f_unidad').val() ,function(data){
                    ...
                    $('#reporte').fadeIn("fast").load('reporte.jsp?f_mes='+$('#f_mes').val()+'&f_unidad='+$('#f_servicio').val()+'&f_anio='+$('#f_anio').val()+'&f_id_personal='+$('#f_id_personal').val()+'&f_actividad=19', function(){ $("#cargando").css("display", "none");});
                });

                $.post('actividad.jsp' ,function(data2){
                    $("#f_actividad").show();
                    $("#f_actividad").html(data2);
                   
                });

                $('#reporte').fadeIn("fast").load('reporte.jsp?f_mes='+$('#f_mes').val()+'&f_unidad='+$('#f_servicio').val()+'&f_anio='+$('#f_anio').val()+'&f_id_personal='+$('#f_id_personal').val()+'&f_actividad='+$('#f_actividad').val(), function(){ $("#cargando").css("display", "none");});

            });

            function SendToExcel(){
                window.open( 'pdf.jsp?f_mes='+$('#f_mes').val()+'&f_unidad='+$('#f_servicio').val()+'&f_anio='+$('#f_anio').val()+'&f_id_personal='+$('#f_id_personal').val()+'&f_actividad='+$('#f_actividad').val() );
                return false;
            }

            function SendToPrint(){
                window.open( 'print.jsp?f_mes='+$('#f_mes').val()+'&f_unidad='+$('#f_servicio').val()+'&f_anio='+$('#f_anio').val()+'&f_id_personal='+$('#f_id_personal').val()+'&f_actividad='+$('#f_actividad').val() );
                return false;
            }
            ...
            function pressMes(){
                var vcl = $(this).val();
                    ...
                    $("#f_nomb").val('');
                    $('#reporte').fadeIn("fast").load('reporte.jsp?f_anio='+$('#f_anio').val()+'&f_unidad='+$('#f_servicio').val()+'&f_mes='+vcl+'&f_id_personal='+$('#f_id_personal').val()+'&f_actividad='+$('#f_actividad').val(), function(){ $("#cargando").css("display", "none");});
            }
            ...
            function pressServicio(){
                var vcl = $(this).val();
                    ...
                    $("#cargando").css("display", "inline");
                    $("#f_nomb").val('');
                    $('#reporte').fadeIn("fast").load('reporte.jsp?f_anio='+$('#f_anio').val()+'&f_unidad='+vcl+'&f_mes='+$('#f_mes').val()+'&f_id_personal='+$('#f_id_personal').val()+'&f_actividad='+$('#f_actividad').val(), function(){ $("#cargando").css("display", "none");});
            } 

            function pressActividad(){
                var vcl = $(this).val();
                    $('#ce').hide("fast");
                    $('#co').show("fast");
                    $('#to').attr("selected","selected");
                    ...
                    $("#cargando").css("display", "inline");
                    $("#f_nomb").val('');
                    $('#reporte').fadeIn("fast").load('reporte.jsp?f_anio='+$('#f_anio').val()+'&f_unidad='+$('#f_servicio').val()+'&f_mes='+$('#f_mes').val()+'&f_id_personal='+$('#f_id_personal').val()+'&f_actividad='+vcl, function(){ $("#cargando").css("display", "none");});
            }
            ...
            function pressAnio(){
                var vcl = $(this).val();
                    ...
                    $("#cargando").css("display", "inline");
                    $("#f_nomb").val('');
                    $('#reporte').fadeIn("fast").load('reporte.jsp?f_mes='+$('#f_mes').val()+'&f_unidad='+$('#f_servicio').val()+'&f_anio='+vcl+'&f_id_personal='+$('#f_id_personal').val()+'&f_actividad='+$('#f_actividad').val(), function(){ $("#cargando").css("display", "none");});
            }
                ...
                <div class="table-primary">
                    <div align="center"  class="table-header">
                        <div class="table-caption">
                            REPORTE DE ACTIVIDADES DE OBSTETRICIA
                        </div>
                    </div>
                    ...
                    <div class="panel-body">
                        ...
                        <div class="input-daterange input-group" id="bs-datepicker-range">
                            ...
                            <span class="input-group-addon">
								<a title="Imprimir Rol Actividad" href="#" id="print" style="border-color:#FFF"><img alt="Imprimir Rol Actividad" src="../images/printer.gif" width="17" height="17" /></a>
							</span>
                    ...
                    %>
                            ...
                            <span class="input-group-addon"> ACTIVIDAD </span>
                                <select class="input-sm form-control" name="f_actividad" id="f_actividad">
